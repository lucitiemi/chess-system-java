...
public class ChessMatch {
	...
	public ChessMatch() {
		...
		check = false;							// nao precisaria, pois por padrao uma propriedade booleana começa sempre como falso
		initialSetup();
	}
	...
	// faz o movimento (faz as verificacoes necessarias)
	...
	// metodo auxiliar para o metodo replacePromotedPiece (instancia a nova peça)
	...
	// testa se o rei esta em checkmate
				...
				for (int i=0; i<board.getRows(); i++) {
					for (int j=0; j<board.getColumns(); j++) {
						if (mat[i][j]) {
							...
							Position targert = new Position(i, j);
							Piece capturedPiece = makeMove(source, targert);
							boolean testCheck = testCheck(color);
							undoMove(source, targert, capturedPiece);
							if (!testCheck) {
								return false;
							}
						}
